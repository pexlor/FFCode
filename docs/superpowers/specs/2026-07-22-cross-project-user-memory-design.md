...
FFCode 当前的系统提示词只包含静态规则和运行环境。新的会话不会记得用户在其他代码
项目中已经确认过的开发偏好、工作习惯和技术背景，因此用户需要反复说明测试方式、代码
风格、协作习惯和常用工具。

本设计为 FFCode 增加一套仅服务代码工作的用户级长期记忆。系统可以在对话中发现候选
...
默认目录为 `~/.ffcode/memory/`。它属于 FFCode/FFCode，不与通用聊天助手共享，因此满足
