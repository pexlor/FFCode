...
本目录实现所有 Tool 的统一授权入口，包括策略匹配、路径边界、Shell 风险分析、用户确认和审计记录。

## 目录边界

- `tool.Executor` 必须在真正执行 Tool 前调用 `PermissionManager`。
- 默认策略遵循最小权限和默认拒绝原则。
- 权限系统只判断操作能否执行，不负责 Tool 的具体业务实现。
- 终端确认器是可替换依赖，核心授权流程不能直接操作 UI。
