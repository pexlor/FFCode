...
type ToolArtifact = contextmanager.ToolArtifact
type SummarySnapshot = contextmanager.SummarySnapshot

var (
	ErrInvalidIdentifier      = contextmanager.ErrInvalidIdentifier
	ErrSummaryVersionConflict = contextmanager.ErrSummaryVersionConflict
	ErrArtifactHashMismatch   = contextmanager.ErrArtifactHashMismatch
	ErrSessionNotFound        = contextmanager.ErrSessionNotFound
	ErrSessionExists          = contextmanager.ErrSessionExists
	ErrUnsafeSessionPath      = contextmanager.ErrUnsafeSessionPath
)

// sessionManifest is the atomic pointer to the active summary and transcript
// metadata. Its JSON shape is intentionally unchanged from the previous store.
...
// New 创建基于本地文件的 ConversationStore。
...
func (s *Store) AppendMessage(ctx context.Context, message StoredMessage) error {
...
func (s *Store) CreateSession(ctx context.Context, metadata SessionMetadata) error {
	if err := ctx.Err(); err != nil {
		return err
	}
...
func (s *Store) GetSession(ctx context.Context, sessionID string) (SessionMetadata, error) {
	if err := ctx.Err(); err != nil {
...
func (s *Store) ListSessions(ctx context.Context, workspace string, limit int) ([]SessionMetadata, error) {
	if err := ctx.Err(); err != nil {
		return nil, err
	}
...
func (s *Store) ListMessagesAfter(ctx context.Context, sessionID, messageID string) ([]StoredMessage, error) {
...
func (s *Store) SaveToolArtifact(ctx context.Context, artifact ToolArtifact, content io.Reader) error {
	if err := ctx.Err(); err != nil {
		return err
	}
...
func (s *Store) LoadToolArtifact(ctx context.Context, sessionID, artifactID string) (ToolArtifact, io.ReadCloser, error) {
	if err := ctx.Err(); err != nil {
...
func (s *Store) ActiveSummary(ctx context.Context, sessionID string) (*SummarySnapshot, error) {
	if err := ctx.Err(); err != nil {
		return nil, err
	}
	if !validIdentifier(sessionID) {
...
func (s *Store) CommitSummary(ctx context.Context, summary SummarySnapshot, expectedVersion int) error {
	if err := ctx.Err(); err != nil {
		return err
	}
